...
class Input(InputInterface):
    ...
    def __call__(self, is_call):
        if is_call:
            _tokens = NewsTokenizer.tokenization(self.data['news_data'])
            news_factors = TokenSRL.labeling(_tokens)
            stock_factors = StockIndex.create_factor(self.data['stock_history'],
                                                     self.data['market_history'],
                                                     self.data['balance_sheet'],
                                                     self.data['shares_outstanding'])
            sentiment_factor = SentimentClassifier.create_factor({
